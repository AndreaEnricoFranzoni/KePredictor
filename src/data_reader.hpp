#ifndef KE_READ_DATA_HPP
#define KE_READ_DATA_HPP
...
template<typename T> 
std::pair<KE_Traits::StoringMatrix,std::vector<int>>
reader_data(Rcpp::NumericMatrix X,
            REM_NAN MA_t)
{
  std::cout << "Kernel Estimate predictor with CV for retained number of PCs 2" << std::endl;
  ...
  if(MA_t == REM_NAN::NR)
  {
    KE_Traits::StoringMatrix x = Eigen::Map<KE_Traits::StoringMatrix>(X.begin(),n_row,n_col);
  ...
  KE_Traits::StoringMatrix x = Eigen::Map<KE_Traits::StoringMatrix>(X_clean.begin(),rows_to_be_retained.size(),n_col);
